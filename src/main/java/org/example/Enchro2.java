 ...
 * aber: klasse darf kein ü oder sonsiges im namen haben!
 *
 ...
 *
 * Enchro2: development in Java17 language level 17, IntelliJ
 * with Help from chatGPT
 */
//only works in folder which is asserted in pom -> javafx:compile,run
public class Enchro2 extends Application {
...
//        ergebnistext.setText("12");
        ...
        Text copyright = new Text("                      © Jan Füllhase 2023");
        Text headline = new Text("Text eingeben:");
        fragegeburtsort = new Text("Jans Geburtsort:");
        fragepasswort = new Text("6 Zeichen Passwort:");  //MUSS 6 zeichen sein!
...
//        stage.setAlwaysOnTop(true);
        ...
        stage.setTitle("Enchro2 - Verschlüsseln und Entschlüsseln");
    ...
    public static String buttonhandler(String ttext1,String ttextfeld1,String ttextfeld2,int type) throws Exception {
        ...
        //case felder leer, default key
        if ((ttextfeld1).isBlank() && ttextfeld2.isBlank()) {
            ttextfeld1 = "redacted";   //TODO put redacted back
            ttextfeld2 = "redacted";
        }

        System.out.println("\n"+"1   "+ttextfeld1 + ttextfeld2);
        if (type == 1) {
            ergebnistext.setText(encrypt(ttext1,ttextfeld1+ttextfeld2));
        }
        if (type == 2) {
//            decryptRotation(ttext1,ttextfeld1+ttextfeld2);
            ergebnistext.setText(decrypt(ttext1,ttextfeld1+ttextfeld2));
        }
...
//                ergebnistext.setText("9 Walziger Enigma, anhand von hashcode.\nHashcode iteriert durch seine Ziffern und wälzt Zahl für Zahl.");
                ergebnistext.setText("Verschlüsselung mit RSA - 16bytes");
            }
    ...
    /**
     * Die idee von Enchro2 ist die benutzung einer java library für die verschlüsselung
     * so entstehen weniger bugs
     *
     * https://stackoverflow.com/questions/10494764/input-length-must-be-multiple-of-16-when-decrypting-with-padded-cipher
     *
     * @param data  message
     * @param secretKey password / key
     * @return
     */
    public static String encrypt(final String data, final String secretKey) {

        // AES = 16 bytes , if smaller than 16 bytes fill with x
        String secret = secretKey.substring(0, Math.min(secretKey.length(), 16));
        secret = String.format("%-16s", secret).replace(' ', 'x');


        System.out.println("debugencryption: this is the key: "+ secret);
    ...
    public static String decrypt(final String encryptedString, final String secretKey) {

        // AES = 16 bytes , if smaller than 16 bytes fill with x
        String secret = secretKey.substring(0, Math.min(secretKey.length(), 16));
        secret = String.format("%-16s", secret).replace(' ', 'x');

        System.out.println("debugdecryption: this is the key: "+ secret);
        ...
        try {
            ...
            System.out.println("FEHLER BEIM ENTSCHLÜSSELN- FALSCHES PASSWORT :/");
            ergebnistext.setText("FEHLER BEIM ENTSCHLÜSSELN- FALSCHES PASSWORT :/");
            throw new RuntimeException(
    ...
    public static String encryptRotation1(String strClearText,String strKey) throws Exception {
        int rot = 0;
        System.out.println("strKey: "+strKey+strKey.hashCode());
        ...
        for (int i = 0; i < strClearText.length(); i++) {
            ...
            //enigma
            //j muss hier immer 7 sein (6 geht besser)
            if (j >= 6) {
                j = 1;
            }
            rot = rotarray[j];
...
//                c += (char) 125 - 32 + 1;               //verschlüsselungs umbruch, um die gesuchten 32-125 Zeichen einzuhalten
    ...
    public static String decryptRotation1(String strEncrypted,String strKey) throws Exception {
        //char array hashcode
        int rot = 0;
